...
namespace Main.ReactiveUI
{
    ...
    public class ViewModelProvidingViewLocator : IViewLocator
    {
        /// <summary>
        ...
        public ViewModelProvidingViewLocator(Func<string, string>? viewModelToViewFunc = null) =>
        ...
        public IViewFor? ResolveView<T>(T? viewModel, string? contract = null)
        {
            ...
            object oViewModel = viewModel ?? throw new ArgumentNullException(nameof(viewModel));

            var view = AttemptViewResolutionFor(viewModel.GetType(), contract, oViewModel);
            ...
            view = AttemptViewResolutionFor(typeof(T), contract, oViewModel);
            ...
            view = AttemptViewResolutionFor(ToggleViewModelType(viewModel.GetType()), contract, oViewModel);
            ...
            view = AttemptViewResolutionFor(ToggleViewModelType(typeof(T)), contract, oViewModel);
        ...
        private IViewFor? AttemptViewResolutionFor<T>(Type? viewModelType, string? contract, T viewModel) where T: class
        {
            ...
            var proposedViewFuncTypeName = typeof(Func<,>).MakeGenericType(viewModelType, typeof(IViewFor<>).MakeGenericType(viewModelType)).AssemblyQualifiedName;
            view = AttemptViewResolution<T>(proposedViewFuncTypeName, contract, viewModel);
            
            return view;
        }

        private IViewFor? AttemptViewResolution<T>(string proposedViewFuncTypeName, string? contract, T viewModel) where T : class
        {
            try
            {
                var viewFuncType = Reflection.ReallyFindType(proposedViewFuncTypeName, throwOnFailure: false);
                if (viewFuncType is null)
                {
                    return null;
                }

                var service = Locator.Current.GetService(viewFuncType, contract);
                ...
                this.Log().Debug(CultureInfo.InvariantCulture, "Resolved service type '{0}'", viewFuncType.FullName);

                var del = (Delegate)service;
                var view = del.DynamicInvoke(viewModel);
                
                return (IViewFor?) view;
            }
            catch (Exception ex)
            {
                this.Log().Error(ex, $"Exception occurred whilst attempting to resolve type {proposedViewFuncTypeName} into a view.");
                throw;
            }
